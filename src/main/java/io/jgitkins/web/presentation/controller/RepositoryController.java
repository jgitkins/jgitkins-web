...
public class RepositoryController {
    ...
    public String newRepository(Authentication authentication, Model model) {
        ...
        RepositoryCreateInitData initData = repositoryCreateFacadeUseCase.getInitData(profile, form.getOwnerType(), form.getOrganizeId());
        createViewSupport.populateModel(model, initData, form, null);
    ...
    public String createRepository(@Valid @ModelAttribute("form") RepositoryCreateForm form,
        ...
        if (validationError != null) {
            createViewSupport.populateModel(model, initData, form, validationError);
            return "repositories/new";
        }

        RepositoryCreateResult result = repositoryCreateFacadeUseCase.createRepository(createViewSupport.toRequest(form, profile));
        if (result.errorMessage() != null) {
            createViewSupport.populateModel(model, initData, form, result.errorMessage());
            return "repositories/new";
        }
    ...
    public String repositoryDetailPage(@PathVariable("namespace") String namespace,
        ...
        if (accessSupport.requiresNotFoundForUnauthenticatedPrivate(detail, userProfileResolver.isAuthenticated(authentication))) {
